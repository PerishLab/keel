...
#[test]
fn veiled_units_stay_off_the_plan_projection_but_open_to_faces() {
    ...
    let sudo = core.sudo();
    let ada = sudo.put("Actor", &[("login", "ada")]).expect("actor");
    let pass = sudo
        .put("Pass", &[("hash", "argon"), ("actor", &ada.to_string())])
        .expect("ceremony write still works on a veiled unit");
    let pack = sudo.query("from Pass").expect("face query works");
